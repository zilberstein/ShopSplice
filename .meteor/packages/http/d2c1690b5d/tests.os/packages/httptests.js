...
var respond = function(req, res) {                                                              // 3
  ...
  if (req.url.slice(0,5) === "/slow") {                                                         // 5
    ...
    res.end("SOME SORT OF SERVER ERROR. " +                                                     // 13
    ...
    return;                                                                                     // 15
  } else if (req.url === "/redirect") {                                                         // 16
    res.statusCode = 301;                                                                       // 17
    // XXX shouldn't be redirecting to a relative URL, per HTTP spec,                           // 18
    // but browsers etc. seem to tolerate it.                                                   // 19
    res.setHeader("Location", TEST_RESPONDER_ROUTE+"/foo");                                     // 20
    res.end("REDIRECT TO FOO");                                                                 // 21
    return;                                                                                     // 22
  } else if (req.url.slice(0,6) === "/login") {                                                 // 23
    var username = 'meteor';                                                                    // 24
    // get password from query string                                                           // 25
    var password = req.url.slice(7);                                                            // 26
    // realm is displayed in dialog box if one pops up, avoid confusion                         // 27
    var realm = TEST_RESPONDER_ROUTE+"/login";                                                  // 28
    var validate = function(user, pass) {                                                       // 29
      return user === username && pass === password;                                            // 30
    };                                                                                          // 31
    var checker = WebApp.__basicAuth__(validate, realm);                                        // 32
    var success = false;                                                                        // 33
    checker(req, res, function() {                                                              // 34
      success = true;                                                                           // 35
    });                                                                                         // 36
    if (! success)                                                                              // 37
      return;                                                                                   // 38
  } else if (req.url === "/headers") {                                                          // 39
    res.statusCode = 201;                                                                       // 40
    res.setHeader("A-Silly-Header", "Tis a");                                                   // 41
    res.setHeader("Another-Silly-Header", "Silly place.");                                      // 42
    res.end("A RESPONSE WITH SOME HEADERS");                                                    // 43
    return;                                                                                     // 44
  }                                                                                             // 45
                                                                                                // 46
  var chunks = [];                                                                              // 47
  req.setEncoding("utf8");                                                                      // 48
  req.on("data", function(chunk) {                                                              // 49
    chunks.push(chunk); });                                                                     // 50
  req.on("end", function() {                                                                    // 51
    var body = chunks.join('');                                                                 // 52
                                                                                                // 53
    if (body.charAt(0) === '{') {                                                               // 54
      body = JSON.parse(body);                                                                  // 55
    }                                                                                           // 56
                                                                                                // 57
    var response_data = {                                                                       // 58
      method: req.method,                                                                       // 59
      url: req.url,                                                                             // 60
      headers: req.headers,                                                                     // 61
      body: body                                                                                // 62
    };                                                                                          // 63
    var response_string = "";                                                                   // 64
    if (req.method !== "HEAD")                                                                  // 65
      response_string = JSON.stringify(response_data);                                          // 66
                                                                                                // 67
    res.statusCode = 200;                                                                       // 68
    res.setHeader("Content-Type", "application/json; charset=utf-8");                           // 69
    res.end(response_string);                                                                   // 70
  });                                                                                           // 71
                                                                                                // 72
};                                                                                              // 73
                                                                                                // 74
var run_responder = function() {                                                                // 75
  WebApp.connectHandlers.stack.unshift(                                                         // 76
    { route: TEST_RESPONDER_ROUTE, handle: respond });                                          // 77
};                                                                                              // 78
                                                                                                // 79
run_responder();                                                                                // 80
                                                                                                // 81
