...
Spark.list = function (cursor, itemFunc, elseFunc) {                                               // 914
    ...
    movedBefore: function (id, before) {                                                           // 1053
      later(function () {                                                                          // 1054
        ...
        if (before === null) {                                                                     // 1056
          ...
          itemDict.get(before).liveRange.insertBefore(frag);                                       // 1060
        }                                                                                          // 1061
        itemDict.moveBefore(id, before);                                                           // 1062
        notifyParentsRendered();                                                                   // 1063
      });                                                                                          // 1064
    },                                                                                             // 1065
                                                                                                   // 1066
    changed: function (id, fields) {                                                               // 1067
      later(function () {                                                                          // 1068
        var elt = itemDict.get(id);                                                                // 1069
        if (!elt)                                                                                  // 1070
          throw new Error("Unknown id for changed: " + id);                                        // 1071
        applyChanges(elt.doc, fields);                                                             // 1072
        Spark.renderToRange(elt.liveRange,                                                         // 1073
                            _.bind(itemFunc, null, transformedDoc(elt.doc)));                      // 1074
      });                                                                                          // 1075
    }                                                                                              // 1076
  });                                                                                              // 1077
                                                                                                   // 1078
  return html;                                                                                     // 1079
                                                                                                   ...
                                                                                                   // 1081
/******************************************************************************/                   // 1082
/* Labels and landmarks                                                       */                   // 1083
/******************************************************************************/                   // 1084
                                                                                                   // 1085
var nextLandmarkId = 1;                                                                            // 1086
                                                                                                   // 1087
Spark.Landmark = function () {                                                                     // 1088
  this.id = nextLandmarkId++;                                                                      // 1089
  this._range = null; // will be set when put onscreen                                             // 1090
};                                                                                                 // 1091
                                                                                                   // 1092
_.extend(Spark.Landmark.prototype, {                                                               // 1093
  firstNode: function () {                                                                         // 1094
    return this._range.firstNode();                                                                // 1095
  },                                                                                               // 1096
  lastNode: function () {                                                                          // 1097
    return this._range.lastNode();                                                                 // 1098
  },                                                                                               // 1099
  find: function (selector) {                                                                      // 1100
    var r = this._range;                                                                           // 1101
    return DomUtils.findClipped(r.containerNode(), selector,                                       // 1102
                                r.firstNode(), r.lastNode());                                      // 1103
  },                                                                                               // 1104
  findAll: function (selector) {                                                                   // 1105
    var r = this._range;                                                                           // 1106
    return DomUtils.findAllClipped(r.containerNode(), selector,                                    // 1107
                                   r.firstNode(), r.lastNode());                                   // 1108
  },                                                                                               // 1109
  hasDom: function () {                                                                            // 1110
    return !! this._range;                                                                         // 1111
  }                                                                                                // 1112
});                                                                                                // 1113
                                                                                                   // 1114
Spark.UNIQUE_LABEL = ['UNIQUE_LABEL'];                                                             // 1115
                                                                                                   // 1116
// label must be a string.                                                                         // 1117
// or pass label === null to not drop a label after all (meaning that                              // 1118
// this function is a noop)                                                                        // 1119
//                                                                                                 // 1120
Spark.labelBranch = function (label, htmlFunc) {                                                   // 1121
  var renderer = currentRenderer.get();                                                            // 1122
  if (! renderer || label === null)                                                                // 1123
    return htmlFunc();                                                                             // 1124
                                                                                                   // 1125
  if (label === Spark.UNIQUE_LABEL)                                                                // 1126
    label = Random.id();                                                                           // 1127
                                                                                                   // 1128
  renderer.currentBranch.pushLabel(label);                                                         // 1129
  var html = htmlFunc();                                                                           // 1130
  var occupied = renderer.currentBranch.getNotes().occupied;                                       // 1131
  renderer.currentBranch.popLabel();                                                               // 1132
                                                                                                   // 1133
  if (! occupied)                                                                                  // 1134
    // don't create annotation if branch doesn't contain any landmarks.                            // 1135
    // if this label isn't on an element-level HTML boundary, then that                            // 1136
    // is certainly the case.                                                                      // 1137
    return html;                                                                                   // 1138
                                                                                                   // 1139
  return renderer.annotate(                                                                        // 1140
    html, ANNOTATION_LABEL, { label: label });                                                     // 1141
                                                                                                   // 1142
  // XXX what happens if the user doesn't use the return value, or                                 // 1143
  // doesn't use it directly, eg, swaps the branches of the tree                                   // 1144
  // around? "that's an error?" the result would be that the apparent                              // 1145
  // branch path of a landmark at render time would be different from                              // 1146
  // its apparent branch path in the actual document. seems like the                               // 1147
  // answer is to have labelBranch not drop an annotation, and keep                                // 1148
  // the branch label info outside of the DOM in a parallel tree of                                // 1149
  // labels and landmarks (likely similar to the one we're already                                 // 1150
  // keeping?) a little tricky since not every node in the label tree                              // 1151
  // is actually populated with a landmark? (though we could change                                // 1152
  // that I guess -- they would be landmarks without any specific DOM                              // 1153
  // nodes?)                                                                                       // 1154
};                                                                                                 // 1155
                                                                                                   // 1156
Spark.createLandmark = function (options, htmlFunc) {                                              // 1157
  var renderer = currentRenderer.get();                                                            // 1158
  if (! renderer) {                                                                                // 1159
    // no renderer -- create and destroy Landmark inline                                           // 1160
    var landmark = new Spark.Landmark;                                                             // 1161
    options.created && options.created.call(landmark);                                             // 1162
    var html = htmlFunc(landmark);                                                                 // 1163
    options.destroyed && options.destroyed.call(landmark);                                         // 1164
    return html;                                                                                   // 1165
  }                                                                                                // 1166
                                                                                                   // 1167
  // Normalize preserve map                                                                        // 1168
  var preserve = {};                                                                               // 1169
  if (_.isArray(options.preserve))                                                                 // 1170
    _.each(options.preserve, function (selector) {                                                 // 1171
      preserve[selector] = true;                                                                   // 1172
    });                                                                                            // 1173
  else                                                                                             // 1174
    preserve = options.preserve || {};                                                             // 1175
  for (var selector in preserve)                                                                   // 1176
    if (typeof preserve[selector] !== 'function')                                                  // 1177
      preserve[selector] = function () { return true; };                                           // 1178
                                                                                                   // 1179
  renderer.currentBranch.mark('occupied');                                                         // 1180
  var notes = renderer.currentBranch.getNotes();                                                   // 1181
  var landmark;                                                                                    // 1182
  if (notes.originalRange) {                                                                       // 1183
    if (notes.originalRange.superceded)                                                            // 1184
      throw new Error("Can't create second landmark in same branch");                              // 1185
    notes.originalRange.superceded = true; // prevent destroyed(), second match                    // 1186
    landmark = notes.originalRange.landmark; // the old Landmark                                   // 1187
  } else {                                                                                         // 1188
    landmark = new Spark.Landmark;                                                                 // 1189
    if (options.created) {                                                                         // 1190
      // Run callback outside the current Spark.isolate's deps context.                            // 1191
      Deps.nonreactive(function () {                                                               // 1192
        options.created.call(landmark);                                                            // 1193
      });                                                                                          // 1194
    }                                                                                              // 1195
  }                                                                                                // 1196
  notes.landmark = landmark;                                                                       // 1197
                                                                                                   // 1198
  var html = htmlFunc(landmark);                                                                   // 1199
  return renderer.annotate(                                                                        // 1200
    html, ANNOTATION_LANDMARK, function (range) {                                                  // 1201
      if (! range) {                                                                               // 1202
        // annotation not used                                                                     // 1203
        options.destroyed && options.destroyed.call(landmark);                                     // 1204
        return;                                                                                    // 1205
      }                                                                                            // 1206
                                                                                                   // 1207
      _.extend(range, {                                                                            // 1208
        preserve: preserve,                                                                        // 1209
        constant: !! options.constant,                                                             // 1210
        rendered: options.rendered || function () {},                                              // 1211
        destroyed: options.destroyed || function () {},                                            // 1212
        landmark: landmark,                                                                        // 1213
        finalize: function () {                                                                    // 1214
          if (! this.superceded) {                                                                 // 1215
            this.landmark._range = null;                                                           // 1216
            this.destroyed.call(this.landmark);                                                    // 1217
          }                                                                                        // 1218
        }                                                                                          // 1219
      });                                                                                          // 1220
                                                                                                   // 1221
      landmark._range = range;                                                                     // 1222
      renderer.landmarkRanges.push(range);                                                         // 1223
      // Help GC avoid an actual memory leak (#1157) by nulling the                                // 1224
      // `renderer` local variable, which holds data structures about                              // 1225
      // the preservation and patching performed during this rendering                             // 1226
      // pass, including references to the old LiveRanges.  If                                     // 1227
      // `renderer` is retained by the LiveRange we initialize here,                               // 1228
      // it creates a chain linking the new LiveRanges to the                                      // 1229
      // renderer, to the old LiveRanges, to the old renderer, etc.                                // 1230
      //                                                                                           // 1231
      // The reason the new LiveRange might retains `renderer` has to                              // 1232
      // do with how V8 implements closures.  V8 considers                                         // 1233
      // `range.finalize` to close over `renderer`, even though it                                 // 1234
      // doesn't use it.  Because `renderer` is used by *some* nested                              // 1235
      // closure, it apparently is retained by all nested closures as                              // 1236
      // part of `Spark.createLandmark`'s function context.                                        // 1237
      renderer = null;                                                                             // 1238
    });                                                                                            // 1239
};                                                                                                 // 1240
                                                                                                   // 1241
SparkTest.getEnclosingLandmark = function (node) {                                                 // 1242
  var range = findRangeOfType(ANNOTATION_LANDMARK, node);                                          // 1243
  return range ? range.landmark : null;                                                            // 1244
};                                                                                                 // 1245
                                                                                                   // 1246
/////////////////////////////////////////////////////////////////////////////////////////////////////
